...
contract rToken is ERC20 {
    ...
    // receive '_underlyingToken' that the receipt token rToken represents (AAVE, UNI, or WETH address)
    // and '_name' + '_symbol' for the receipt token (rToken)
    constructor(address _underlyingToken,  string memory _name, string memory _symbol) public
    
    // Initialize the contructor of the inherited ERC20 contract to deploy the rToken (rAAVE, rUNI, or rWETH)
    ERC20(_name, _symbol) {
    
     // Safety Check: Ensure the underlying token is not the zero address
    require(underlyingToken != address(0), "Underlying token cannot be the zero address.");

    // Set the owner of the contract to the msg.sender
    owner = msg.sender;
    
    // Store _underlyingToken 
    underlyingToken = _underlyingToken;
    }
    
     // Mint rTokens, with onlyOwner modifier
    function mint(address _to, uint256 _amount) external onlyOwner {
        _mint(_to, _amount);
    }

    // Burn rTokens, with onlyOwner modifier
    function burn(address _from, uint256 _amount) external onlyOwner {
        _burn(_from, _amount);
    }
}
